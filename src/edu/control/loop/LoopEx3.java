...
public class LoopEx3 {
    ...
    public void method1(){
        ...
        // str 문자열의 총 길이를 length() 메서드를 활용해 가져오고
     ...
     * while문이 false여도 최초 1회는 무조건 조건이 true일 때 식을 실행
     */
    /*
        0이 입력될 때 까지 정수를 계속 입력받고 0 입력 시 누적된 합계 출력
     */
    public void method4(){
        int sum = 0;
        int input = 0;
        ...
        do { // 입력한 값이 0이 아닐 때 반복
            ...
            input = sc.nextInt();
            sum += input;
        } while (input != 0); // input이 0이 아닐 때 반복하기 위한 조건 작성
     ...
     */
    public void method5(){
        System.out.println("문장을 입력하면 거꾸로 출력됩니다.");
        System.out.print("문장을 입력하세요 : ");
        String str = sc.nextLine();

        // 문자열의 길이-1 부터 0까지 감소하며 출력
        // index는 0부터 시작하지만 length는 1부터 시작하기 때문에 -1.
     ...
     *
     * 1 ~ 10 사이의 랜덤 숫자를 맞출 때 까지 반복
     * Math.random() + 난수 -> 정수로 형변환  활용
     *
     * 변수이름 : int answer  input
     *
     * 만약에 정답보다 사용자가 입력한 값이 크다면 -> "작성한 숫자보다 작습니다."
     *       정답보다 사용자가 입력한 값이 작다면 -> "작성한 숫자보다 큽니다."
     *       정답이라면 -> "정답입니다!"
     */
    public void method6(){
        ...
        System.out.println("1 ~ 10 사이의 랜덤 숫자를 맞춰보세요!");

        while(true) {
            System.out.print("숫자를 입력하세요 : ");
            input = sc.nextInt();
            count++; // 입력 시도할 때 마다 자동으로 +1 씩 증가한 후 변수이름에 시도횟수 저장

            if (input > answer) System.out.println("작성한 숫자보다 작습니다.");
            else if (input < answer) System.out.println("작성한 숫자보다 큽니다.");
            else {
                // 중괄호 내에 구문이 2개 이상일 경우에는 중괄호 생략 불가
    ...
    public void method7(){
        ...
        // 예외 처리 (try-catch)를 이용해서 추후 문제가 발생하지 않도록 처리할 예정
    ...
    /**
     * 비밀번호 : "java1234"
     * 사용자가 올바른 비밀번호를 입력할 때 까지 입력받음
     * 입력횟수가 5회를 초과하면 "접속 차단합니다" 메세지를 출력하고 종료
     * do-while문 활용, while(true) 조건 - 입력횟수가 5회 초과 시 break 활용
     * 시도횟수(0/5) 비밀번호를 입력하세요 :
     * method8()
     * if (input.equals(checkPassword)){}  : 문자열을 비교했을 때 일치하는지 확인
     *
     * 변수이름 : String checkPassword = "java1234"
     *           String input = "" // 비밀번호 입력하고 입력된 비밀번호 데이터 저장하는 변수이름
     *           int attempts = 0; // 시도횟수
     */
    public void method8(){
        ...
        do {
            System.out.printf("시도횟수(%d / 5)  비밀번호를 입력하세요 : ", attempts);
            input = sc.next();
            ...
            if(attempts > 5) {
                System.out.println("시도횟수를 초과해 접속을 차단합니다.");
                break;
            }
        } while(true);
    }
